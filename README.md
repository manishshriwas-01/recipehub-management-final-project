...
Follow these steps to run RecipeHub locally.
...
Never commit the actual .env file or database credentials or secrets to Git.
...
The backend uses environment variables for MongoDB and JWT configuration.

The repository includes server/.env.example as a safe reference for the required variables. Create your own server/.env file locally and provide the actual values.
